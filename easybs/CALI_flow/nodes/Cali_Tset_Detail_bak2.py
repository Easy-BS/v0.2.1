...
ENABLE_NSAA_POLISH = True # False #True
NSGA_POP = 32
NSGA_GEN = 12
NSGA_SEED = 42
...
def residual_rescue_pass(
    ...
    """
    Manual-like rescue pass:
    for months still underpredicted or overpredicted, try targeted T_away-first fixes.
    """
    best_map = dict(base_map)
    best_cvr = current_best_cvr
    ...
    # prioritize biggest remaining errors
    month_order = sorted(
        MEASURED_MONTHS,
        key=lambda mm: abs(residual.get(mm, 0.0)),
        reverse=True
    ...
    for mm in month_order:
        ...
        if r < -150:  
            # sim too low -> increase heating effect
            candidates += [
            ...
            # sim too high -> decrease heating effect
            candidates += [
                (th, clamp(ta - 1.0, BOUNDS["t_away"][0], BOUNDS["t_away"][1])),
                (th, clamp(ta - 2.0, BOUNDS["t_away"][0], BOUNDS["t_away"][1])),
                (clamp(th - 0.5, BOUNDS["t_home"][0], BOUNDS["t_home"][1]), ta),
        ...
        for idx, (th_try, ta_try) in enumerate(candidates, start=1):
            ...
            # prefer lower CVRMSE, then lower |NMBE|
...
def main() -> None:
    ...
    for it in range(1, MAX_OUTER_ITERS + 1):
        ...
        if cvr_best <= 15.0 and abs(nb_best) <= 5.0:
            ...
            break
    #%%
    # ---- Residual rescue pass (manual-like monthwise correction)
    ...
    if rescue_improved:
        ...
        print(f"[RESCUE] accepted improved solution: CVRMSE={cvr_best:.3f}% NMBE={nb_best:.3f}%")
    else:
